...
export default class NGOScreen1 extends React.Component {
    ...
    getAllRequests = () => {
        this.requestRef = firestore.collection("cases").where("assingedEmail", "==", this.state.email.toUpperCase()).where('status', '==', "Rescued").onSnapshot((snapshot) => {
            var allRequests = [];
                ...
                <View style={{ flex: 1 }}>
                    {this.state.allRequests.length === 0 ? (
                        <View style={styles.beforeText}>
                            <Text style={{ fontSize: 16, color: '#fff' }}>No Animals Save</Text>
                            <Text style={{ fontSize: 14, color: '#fff', textAlign: 'center', width: 300 }}>An List of animals saved by you will be displayed here.</Text>
                        </View>
                    ) : (
                        <FlatList
